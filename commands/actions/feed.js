...
module.exports = class extends (
  ...
  get options() {
    return {
      ...
      aliases: ["кормить", "кормить-ням-ням"],
      permLevel: 0,
    };
  }
  async run(message, args) {
    ...
    if (!user) return message.channel.send("*я хочууу!*");
    if (user.user.id == message.member.id)
      return message.reply(`*прятного аппетита :>*`);
    if (user.user.id == this.client.user.id)
      return message.reply(`<a:ash_hugheart:782575078596149260>`);

    superagent.get(`https://nekos.life/api/v2/img/feed`).then((body) => {
      message.channel.send(
        new MessageEmbed()
          .setAuthor(
            `${message.member.user.username} даёт трапезничать ${user.user.username}`,
            asetsi.client["plus"]
          )
          .setDescription(`Приятного аппетита! >w<`)
